...
<head>
	...
	iconURLs, err := findIconURLsFromHTMLDocument(t.Context(), testURL,
		strings.NewReader(html), "text/html")
	if err != nil {
		t.Fatal(err)
	}
...
<head>
	...
	iconURLs, err := findIconURLsFromHTMLDocument(t.Context(),
		resolveExampleReference(t, "/folder/"), strings.NewReader(html),
		"text/html")
	if err != nil {
		t.Fatal(err)
	}
...
func resolveExampleReference(t *testing.T, path string) *url.URL {
	t.Helper()
	u, err := url.Parse(path)
	require.NoError(t, err)
	require.NotNil(t, u)
	return testURL.ResolveReference(u)
}
...
<head>
	...
	assert.Equal(t, expected, iconURLs)
}
