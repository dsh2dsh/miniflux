...
func (h *handler) showCategoryEntryPage(w http.ResponseWriter, r *http.Request,
	...
	if nextEntry != nil {
		nextEntryRoute = route.Path(h.router, "categoryEntry", "categoryID",
			categoryID, "entryID", nextEntry.ID)
	}

	var prevEntryRoute string
	if prevEntry != nil {
		prevEntryRoute = route.Path(h.router, "categoryEntry", "categoryID",
			categoryID, "entryID", prevEntry.ID)
	}
